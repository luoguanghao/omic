...
source('/home/lgh/my_project/omic/project/hl/flt3/boxplot.R')
# source('/mnt/d/omic/modules/plot/auc.R')
source('/home/lgh/my_project/omic/project/hl/flt3/survival.R')
...
test_gene_ls_in_drug <- function(drug_sen_df, drug, sp_sample_ls, sen_unsen_list, exp_mat, gene_ls, 
    ...
    if(plot_flag==TRUE){
        ...
        phm = pheatmap(
            ...
            #cutree_cols=2, 
            annotation_col  = list_for_pheatmap$anno_nc_df,
            main = list_for_pheatmap$drug
            )
    }
    ## ssGSEA打分

    gene_list = list(gene_ls=gene_ls)
    gsva_matrix <- gsva(as.matrix(exp_mat[,sp_sample_ls]), 
                        gene_list,
                        method='ssgsea', 
                        kcdf='Gaussian', 
                        abs.ranking=TRUE, 
...
test_gene_ls_in_drug_for_Exhaustion <- function(drug_sen_df, drug, sp_sample_ls, sen_unsen_list, exp_mat, gene_ls, 
    ...
    df_plot_with_drug = merge(df_ssgsea_score,drug_sen_df%>%filter(inhibitor==drug),by.x='sample',by.y='lab_id')
    ...
    return(
        list(gsva_matrix=gsva_matrix,
            df_ssgsea_score=df_ssgsea_score,
            df_plot=df_plot,
            df_plot_with_drug=df_plot_with_drug)
        )
}


test_gene_ls_in_drug_for_Exhaustion_bycorr <- function(drug_sen_df, drug, sp_sample_ls, exp_mat, gene_ls, 
                                                auc_ic50='auc',verbose=FALSE,parallel.sz=1){
    # drug_sen_df
    # sp_sample_ls
    # exp_mat
...
compare_3_metric <- function(dir,filename,drug,exp_mat,sp_sample_ls,
    ...
    result_df$cebpa = c(result_df$cebpa,roc1$auc)
    result_df$blast = c(result_df$blast,roc2$auc)
    result_df$npm1 = c(result_df$npm1,roc3$auc)
    
    result_df$cebpa_blast_p = c(result_df$cebpa_blast_p,roc.test(roc1,roc2, reuse.auc=TRUE,boot.n=10000)$p.value)
    result_df$cebpa_npm1_p = c(result_df$cebpa_npm1_p,roc.test(roc1,roc3, reuse.auc=TRUE,boot.n=10000)$p.value)
    
    result_df$cebpa_ci = c(result_df$cebpa_ci,paste(sprintf('%.2f',roc1$ci),collapse=('_')))
    ...
    if(plot_flag==TRUE){
        text = paste(
            c(paste('ssGSEA score=',sprintf('%.2f',roc1$auc),sep=''),
            paste('BLAST=',sprintf('%.2f',roc2$auc),sep=''),
            paste('NPM1=',sprintf('%.2f',roc3$auc),sep='')), collapse='\n'
                    )

        # ggroc(list(ssGSEA_score_cebpa_genes = roc1_2, blast = roc2, npm1 = roc3))+labs(title=drug) + theme_bw()

        p=ggroc(list(ssGSEA_score_cebpa_genes = roc1, blast = roc2, npm1 = roc3),size=1)+ 
                annotate('text', x = 0.4, y = 0.10, alpha = .99, colour = "black",label = text,hjust=0) +
                labs(title=drug, x="Specificity", y="Sensitivity") +
                theme_bw()+theme(text=element_text(size=15,face='bold',family='serif'),plot.title = element_text(hjust = 0.5))
...
compare_pb_bm_blast <- function(dir,filename,drug,exp_mat,sp_sample_ls,
    ...
    result_df$cebpa_blastbm_p = c(result_df$cebpa_blastbm_p,roc.test(roc1,roc2_1, reuse.auc=TRUE,boot.n=10000)$p.value)
    result_df$cebpa_blastpb_p = c(result_df$cebpa_blastpb_p,roc.test(roc1,roc2_2, reuse.auc=TRUE,boot.n=10000)$p.value)
    result_df$cebpa_npm1_p = c(result_df$cebpa_npm1_p,roc.test(roc1,roc3, reuse.auc=TRUE,boot.n=10000)$p.value)
    ...
    p=ggroc(list(ssGSEA_score_cebpa_genes = roc1, blastbm = roc2_1, blastpb = roc2_2, npm1 = roc3))+ 
...
compare_3_metric_each_drug <- function(){
