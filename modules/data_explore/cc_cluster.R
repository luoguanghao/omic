...
gsva_cc_cluster <- function(gene_pathway_df, exp_mat){
...
if(FALSE){
    ...
    result_dir = './result/'
    project_name = sprintf('phospho_subtyping_NA%s', '')


    # 数据导入
    normalized_exp_mat = readxl::read_excel('~/my_project/data/changhai_data/protein_phosph/log2-PHOgroup-sum-median-ref.xlsx')
    colnames(normalized_exp_mat)[1] = 'symbols'

    ## 处理NA值

    normalized_exp_mat[normalized_exp_mat=='NA'] = NA
    normalized_exp_mat[,c(2:dim(normalized_exp_mat)[2])] = sapply(normalized_exp_mat[,c(2:dim(normalized_exp_mat)[2])], as.numeric)
    normalized_exp_mat = normalized_exp_mat%>%column_to_rownames('symbols')
    ###
    ...
    sel_genes_byna = rownames(normalized_exp_mat)[as.vector((apply(is.na(normalized_exp_mat),1,sum)/dim(normalized_exp_mat)[2]<pna))]  ###



    gene_var = apply(normalized_exp_mat[sel_genes_byna,],1,var)
    sel_genes = names(gene_var[base::order(gene_var,decreasing=TRUE)][1:round(dim(normalized_exp_mat)[1]*pn_top)])

    exp_mat_cc = normalized_exp_mat[sel_genes,]
    ...
    ##
    results = ConsensusClusterPlus(t(scale(t(exp_mat_cc))),maxK=maxK,clusterAlg='km',pItem=0.8,reps=1000,
                                title=paste(result_dir,paste(project_name,pn_top,pna,sep='_'),sep=''),plot='pdf')  # 这里设置有讲究
    ...
    # input of ConsensusClusterPlus: gene×sample ,this func is do the cluster for sample
    ##
    results_for_gene = ConsensusClusterPlus(scale(t(exp_mat_cc)),maxK=maxK)
    ...
    for(i in 1:kmax){ ###########
        ...
        write.table(annotation_col,file=paste(result_dir,paste(project_name,pn_top,pna,i,sep='_'),'_cluster_df.tsv',sep=''),sep='\t')
