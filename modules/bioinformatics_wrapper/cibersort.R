...
data <- read_tsv("~/my_project/other_analysis/Proteostasis_aml/AMLrnaseq_DESeq2矫正后_new_nodup_for_cibersort.tsv")

data <- aggregate(.~gene, data, mean)
...
write.table(out,file="symbol.txt",sep="\t",quote=F,col.names=F)        #


res = CIBERSORT(sig_matrix='~/my_project/data/cibersort/LM22.txt',
          mixture_file='./MLrnaseq_DESeq2矫正后_new_nodup_scale.tsv', perm=100, QN=TRUE)
}